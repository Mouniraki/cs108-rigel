...
public enum MoonModel implements CelestialObjectModel<Moon>{
    ...
    private final static double SIN_ORBIT_INCL = Math.sin(ORBIT_INCL);
    private final static double COS_ORBIT_INCL = Math.cos(ORBIT_INCL);
    ...
    private EclipticCoordinates moonEclipticPos(double daysSinceJ2010, double moonOrbitalLon, double sinSunMeanAnomaly){
        ...
        double sinLonDiff = Math.sin(moonOrbitalLon - corrLonAscNode);
        double cosLonDiff = Math.cos(moonOrbitalLon - corrLonAscNode);

        double eclLon = Math.atan2(
                sinLonDiff * COS_ORBIT_INCL,
                cosLonDiff) + corrLonAscNode;

        double eclLat = Math.asin(sinLonDiff * SIN_ORBIT_INCL);
    ...
    private float moonPhase(double moonOrbitalLon, double sunLonEclGeo){
        return (float) ((1 - Math.cos(moonOrbitalLon - sunLonEclGeo)) / 2);
    }
